            ...
            <div class="block border_top_FFFFFF">
                <div class="brod">
                    <ul>
                        ...
                        <li><a href="{{route('legalperson_category')}}">{{__('Юридические лица')}}</a> • </li>
                        ...
                        <div class="left_menu">
                            <ul>
                                @if($items)
                                    <ul class="">
                                        @foreach($items as $it)
                                            <li class="{{ active_linkMenu(asset(route('legalperson_page', $it->slug))) }}"><a class="add__mobile_menu" href="{{route('legalperson_page', $it->slug)}}">{{ ($it->title_menu)?:$it->title }}</a></li>
                                        @endforeach
                                    </ul>
                                @endif
                    ...
                    <div class="re_section__right">
                        ...
                        @if($item->calc)
                            @foreach($item->calc as $name => $calc)
                                @include('pages.leader.legal_person.partial.'. $name, ['calc'=> $calc])
                            @endforeach
                        @endif
                    </div>
